...
import zensharp.symbols.SymbolZenClass;
...
public class ZenParsedFile {
    ...
    public ZenParsedFile(String filename, String classname, ZenTokener tokener, IEnvironmentGlobal environment) {
        ...
        while(tokener.peek() != null && tokener.peek().getType() == ZenTokener.T_IMPORT) {
            Token start = tokener.next();
            
            List<String> importName = new ArrayList<>();
            Token tName = tokener.required(ZenTokener.T_ID, "identifier expected");
            importName.add(tName.getValue());
            
            while(tokener.optional(ZenTokener.T_DOT) != null) {
                Token tNamePart = tokener.required(ZenTokener.T_ID, "identifier expected");
                importName.add(tNamePart.getValue());
            }
            
            String rename = null;
            if(tokener.optional(ZenTokener.T_AS) != null) {
                Token tRename = tokener.required(ZenTokener.T_ID, "identifier expected");
                rename = tRename.getValue();
            }
            
            tokener.required(ZenTokener.T_SEMICOLON, "; expected");
        ...
        while(tokener.hasNext()) {
            ...
            if(next.getType() == ZenTokener.T_GLOBAL || next.getType() == ZenTokener.T_STATIC) {
                ParsedGlobalValue value = ParsedGlobalValue.parse(tokener, environmentScript, classname, next.getType() == ZenTokener.T_GLOBAL);
            ...
            } else if(next.getType() == ZenTokener.T_FUNCTION) {
